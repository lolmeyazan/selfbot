...
import { api, type MessageTemplateInput, type PresencePresetInput } from '@shared/routes';
...
export function useCreateMessageTemplate() {
  ...
  return useMutation({
    mutationFn: async (data: MessageTemplateInput) => {
...
export function useCreatePresencePreset() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: PresencePresetInput) => {
